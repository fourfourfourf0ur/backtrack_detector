...
// This is the primary exported function by a dll, referenced by name via
// dynamic binding that exposes an opqaue function pointer to the interface.
//
...
EXPORT_FUNCTION IBaseInterface *CreateInterface(const char *pName,
                                                int *pReturnCode) {
  InterfaceReg *pCur;
...
CSysModule *Sys_LoadModule(const char *pModuleName) {
  ...
  if (pModuleName[0] != '/') {
    ...
    if (szCwd[strlen(szCwd) - 1] == '/') szCwd[strlen(szCwd) - 1] = '\0';

    _snprintf(szAbsoluteModuleName, sizeof(szAbsoluteModuleName), "%s/%s",
              szCwd, pModuleName);
    hDLL = dlopen(szAbsoluteModuleName, RTLD_NOW);
  } else {
    _snprintf(szAbsoluteModuleName, sizeof(szAbsoluteModuleName), "%s",
              pModuleName);
...
CreateInterfaceFn Sys_GetFactory(CSysModule *pModule) {
  ...
  return reinterpret_cast<CreateInterfaceFn>(
      Sys_GetProcAddress(pModule, CREATEINTERFACE_PROCNAME));
}
...
void *InitializeInterface(char const *interfaceName,
